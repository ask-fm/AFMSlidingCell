...
- (BOOL)panGestureShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
    if (self.cellState == kCoolSlidingStateButtonsHidden) {
        if ([self.delegate respondsToSelector:@selector(shouldAllowShowingButtonsForCell:)] &&
            ![self.delegate shouldAllowShowingButtonsForCell:self])
            return NO;
    }
...
- (void)showButtonViewAnimated:(BOOL)animated
{
    ...
    [UIView animateWithDuration:animationDuration animations:^{
        ...
        if ([self.delegate respondsToSelector:@selector(buttonsDidShowForCell:)])
            [self.delegate performSelector:@selector(buttonsDidShowForCell:) withObject:self];
    }];
}
...
- (void)hideButtonViewAnimated:(BOOL)animated
{
    ...
    [UIView animateWithDuration:animationDuration animations:^{
        ...
        [self.buttonContainer setHidden:YES];
        if ([self.delegate respondsToSelector:@selector(buttonsDidHideForCell:)])
            [self.delegate performSelector:@selector(buttonsDidHideForCell:) withObject:self];
    }];
}
